...
async function ensureFreeze(): Promise<AnyRecord> {
  ...
  if (typeof saved.sourceContextImmutableSha256 === "string") {
    const context = JSON.parse(await readFile(join(here, "source-context.json"), "utf8")) as AnyRecord;
    const immutableContext = {
      immutableSourceScore: context.immutableSourceScore,
      completeSourceMidi: context.completeSourceMidi,
      sourceNoteHash: context.sourceNoteHash,
      sourceMidiNoteCount: context.sourceMidiNoteCount,
      sectionContexts: context.sectionContexts,
    };
    if (sha256(stable(immutableContext)) !== saved.sourceContextImmutableSha256) {
      throw new Error("Frozen immutable source context changed after offline gate.");
    }
  }
...
async function freezeOfflineEvidence(): Promise<AnyRecord> {
  ...
  const evidenceHashes = Object.fromEntries(await Promise.all(
    (["baseline.json", "offline-fixture.json"] as const).map(async (name) =>
  ...
  const updated = {
    ...
    evidenceHashes,
    sourceContextImmutableSha256: sha256(stable(immutableContext)),
