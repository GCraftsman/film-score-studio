...
async function verifyFrozenSource(): Promise<AnyRecord> {
  const [current, frozen, metadataRaw] = await Promise.all([
  ...
  try {
    ...
    const concept = await requestJson({
      ...
      stage: "skill-concept",
       ...
       user: { ...styleContext, activeSkill: "concept", styleSkill: style.payload },
       maxTokens: LARGE_CONTEXT_COMPLETION_TOKENS,
      calls,
      score: baselineCopy,
      sourceMidi,
    ...
    for (const instruction of instructions) {
      ...
      const writer = await requestJson({
        ...
        stage: "instrument-writer",
         agent: "Orchestrator",
         system: `You are the single consolidated Orchestrator's retained ${track.instrument} writer subroutine assigned only to trackId "${track.id}". This is not a separate adviser persona. Read the complete accumulated score, source MIDI, loaded skill documents, style/concept replies, and Orchestrator instruction, but write only add-region operations on this assigned track. Never remove pre-existing regions, change membership, or address another track. Return JSON only: {"summary":"non-empty <=1200 chars","operations":[...]} with canonical operation objects and all notes. ${SEGMENT_RULES} ${SPARSE_WRITING_RULES} Every added region must be <=${MAX_REGION_BEATS} beats and fit inside score duration ${INTRO_BEATS}; split longer phrases rather than truncating. ${playableInstrumentCatalogPrompt()}`,
        user: {
           ...styleContext,
